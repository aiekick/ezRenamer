...
SceneMergerNode::SceneMergerNode() : BaseNode()
{
	...
	m_NodeTypeString = "SCENE_MERGER_NODE";
...
void SceneMergerNode::NeedResizeByResizeEvent(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	ZoneScoped;
...
void SceneMergerNode::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ez::fvec2* vTextureSize)
{	
	ZoneScoped;
...
vk::DescriptorImageInfo* SceneMergerNode::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ez::fvec2* vOutSize)
{
	ZoneScoped;
...
BaseSlotWeak SceneMergerNode::AddPreDefinedInput(const BaseSlot& vBaseSlot)
{
	if (vBaseSlot.slotType == "SHADER_PASS")
	{
		...
		if (slot_ptr)
		{
			...
			BaseNode::freeNodeId = ez::maxi<uint32_t>(BaseNode::freeNodeId, slotID) + 1U;
