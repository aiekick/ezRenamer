...
#include <SoGLSL/Utils/VertexStruct.h>
...
void SceneMergerModule::NeedResizeByResizeEvent(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	ZoneScoped;
...
void SceneMergerModule::NeedResizeByHand(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	ZoneScoped;
...
void SceneMergerModule::SetTexture(const uint32_t& vBindingPoint, vk::DescriptorImageInfo* vImageInfo, ez::fvec2* vTextureSize)
{	
	ZoneScoped;
}
...
vk::DescriptorImageInfo* SceneMergerModule::GetDescriptorImageInfo(const uint32_t& vBindingPoint, ez::fvec2* vOutSize)
{
	ZoneScoped;
...
bool SceneMergerModule::setFromXml(tinyxml2::XMLElement* vElem, tinyxml2::XMLElement* vParent, const std::string& vUserDatas)
{
	...
	if (strParentName == "scene_merger_module")
	{
		if (strName == "can_we_render")
			m_CanWeRender = ez::ivariant(strValue).GetB();
	}
