...
std::function<bool(BaseNodeWeak, tinyxml2::XMLElement*, tinyxml2::XMLElement*,
	const std::string&, const std::string&, const ez::fvec2&, const size_t&)> BaseNode::sLoadNodeFromXMLCallback; // log infos
bool BaseNode::LoadNodeFromXML_Callback(const BaseNodeWeak& vBaseNodeWeak, tinyxml2::XMLElement* vElem, tinyxml2::XMLElement* vParent,
	const std::string& vNodeName, const std::string& vNodeType, const ez::fvec2& vPos, const size_t& vNodeId)
{
...
void BaseNode::InitGraph(const nd::Style& vStyle)
{
...
void BaseNode::ZoomToContent() const
{
	if (m_BaseNodeState.m_NodeGraphContext)
	{
		auto context = reinterpret_cast<nd::Detail::EditorContext*>(m_BaseNodeState.m_NodeGraphContext);
...
bool BaseNode::DrawWidgets(const uint32_t& /*vCurrentFrame*/, ImGuiContext* vContext, const std::string& /*vUserDatas*/)
{
...
bool BaseNode::DrawOverlays(const uint32_t& /*vCurrentFrame*/, const ImRect& /*vRect*/, ImGuiContext* vContext, const std::string& /*vUserDatas*/)
{
	assert(vContext); ImGui::SetCurrentContext(vContext);
...
bool BaseNode::DrawDialogsAndPopups(const uint32_t& /*vCurrentFrame*/, const ImVec2& /*vMaxSize*/, ImGuiContext* vContext, const std::string& /*vUserDatas*/)
{
	assert(vContext); ImGui::SetCurrentContext(vContext);
...
std::string BaseNode::GetAvailableNodeStamp(const std::string& vNodeStamp)
{
	...
	if (m_NodeStamps.find(vNodeStamp) != m_NodeStamps.end()) // trouv�
	{
		...
		res = vNodeStamp + "_" + ez::toStr(m_NodeStamps[vNodeStamp]);
	}
	else
	{
...
void BaseNode::NeedResizeByHand(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	for (const auto& input : m_Inputs)
	{
		if (input.second)
...
void BaseNode::NeedResizeByResizeEvent(ez::ivec2* vNewSize, const uint32_t* vCountColorBuffers)
{
	for (const auto& input : m_Inputs)
	{
		if (input.second)
...
ez::fvec2 BaseNode::GetOutputSize() const
{
	for (const auto&  input : m_Inputs)
	{
		if (input.second)
...
std::string BaseNode::getXml(const std::string& vOffset, const std::string& vUserDatas)
{
	...
	if (!m_ChildNodes.empty())
	{
		...
		res += vOffset + "\t<canvas>\n";
		res += vOffset + "\t\t<offset>" + ez::fvec2(GetCanvasOffset()).string() + "</offset>\n";
		res += vOffset + "\t\t<scale>" + ez::toStr(GetCanvasScale()) + "</scale>\n";
		res += vOffset + "\t</canvas>\n";
				...
				!link.second->out.expired())
			{
				...
				if (inPtr && outPtr)
				{
						...
						!outPtr->parentNode.expired())
					{
						...
						{
							std::string inNodeIdSlotId = ez::toStr("%u:%u", inParentPtr->GetNodeID(), inPtr->GetSlotID());
							std::string outNodeIdSlotId = ez::toStr("%u:%u", outParentPtr->GetNodeID(), outPtr->GetSlotID());
		...
		if (slotLeftPtr)
		{
			...
			{
				outLeftSlot = ez::toStr("%u:%u", slotLeftParentNodePtr->GetNodeID(), slotLeftPtr->GetSlotID());
		...
		if (slotMiddlePtr)
		{
			...
			{
				outMiddleSlot = ez::toStr("%u:%u", slotMiddleParentNodePtr->GetNodeID(), slotMiddlePtr->GetSlotID());
		...
		if (slotRightPtr)
		{
			...
			{
				outRightSlot = ez::toStr("%u:%u", slotRightParentNodePtr->GetNodeID(), slotRightPtr->GetSlotID());
	...
	{
		res += vOffset + ez::toStr("<node name=\"%s\" type=\"%s\" pos=\"%s\" id=\"%u\">\n",
			name.c_str(),
			m_NodeTypeString.c_str(),
			ez::fvec2(pos.x, pos.y).string().c_str(),
			(uint32_t)GetNodeID());
...
bool BaseNode::setFromXml(tinyxml2::XMLElement* vElem, tinyxml2::XMLElement* vParent, const std::string& vUserDatas)
{
	...
	{
		if (strName == "offset")
			SetCanvasOffset(ez::toImVec2(ez::fvariant(strValue).GetV2()));
		else if (strName == "scale")
			SetCanvasScale(ez::fvariant(strValue).GetF());
		...
		if (strName == "node")
		{
			...
			for (const tinyxml2::XMLAttribute* attr = vElem->FirstAttribute(); attr != nullptr; attr = attr->Next())
			{
				...
				if (attName == "name")
					...
					_pos = ez::fvariant(attValue).GetV2();
				else if (attName == "id")
					_nodeId = ez::ivariant(attValue).GetU();
			}
	...
	else if (strParentName == "links")
	{
		if (strName == "link")
		{
			...
			auto vecIn = ez::splitStringToVector(inStr, ':');
			auto vecOut = ez::splitStringToVector(outStr, ':');

			if (vecIn.size() == 2 && vecOut.size() == 2)
			{
				SlotEntry entIn;
				entIn.first = ez::ivariant(vecIn[0]).GetU();
				entIn.second = ez::ivariant(vecIn[1]).GetU();

				SlotEntry entOut;
				entOut.first = ez::ivariant(vecOut[0]).GetU();
				entOut.second = ez::ivariant(vecOut[1]).GetU();
	...
	else if (strParentName == "node")
	{
		...
		if (strName == "slot")
		{
			for (const tinyxml2::XMLAttribute* attr = vElem->FirstAttribute(); attr != nullptr; attr = attr->Next())
			{
				...
				if (attName == "index")
					slot.index = ez::ivariant(attValue).GetU();
	...
	else if (strParentName == "outputs")
	{
		if (strName == "output")
		{
			...
			auto vec = ez::splitStringToVector(ids, ':');

			if (vec.size() == 2)
			{
				SlotEntry ent;
				ent.first = ez::ivariant(vec[0]).GetU();
				ent.second = ez::ivariant(vec[1]).GetU();
