...
struct Func_Loc_In_Code_Struct
{
	...
	std::string stamp;
	std::string sourceFunc;
	std::vector<ez::uvec2> locFunc;
	ez::uvec2 sourceLoc;
	std::string sourceFullFunc;
	std::vector<ez::uvec2> locFullFunc;
	//size_t start;
	//size_t end;
...
public: // static
	...
	static std::function<bool(BaseNodeWeak, tinyxml2::XMLElement*, tinyxml2::XMLElement*,
		const std::string&, const std::string&, const ez::fvec2&, const size_t&)> sLoadNodeFromXMLCallback; // log infos
...
public:
	// only during loading for change slots
...
public: // Code Generation
	ez::uvec2 m_CodeBlock; // x:start / y:end in m_Code
...
public: // gui interface
	...
	bool DrawOverlays(const uint32_t& vCurrentFrame, const ImRect& vRect, ImGuiContext* vContext, const std::string& vUserDatas) override;
	bool DrawDialogsAndPopups(const uint32_t& vCurrentFrame, const ImVec2& vMaxSize, ImGuiContext* vContext, const std::string& vUserDatas) override;
