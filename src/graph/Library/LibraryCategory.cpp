...
#include <SoGLSL/Graph/Library/LibraryCategory.h>
...
void LibraryCategory::AddCustom(const std::string& vCategoryPath, const std::string& vNodeLabel, const std::string& vNodeType, const ez::fvec4& vColor)
{
	if (vCategoryPath.empty()) return;
	if (vNodeLabel.empty()) return;

	auto vec = ez::splitStringToVector(vCategoryPath, '/');
	if (!vec.empty())
	{
		auto cat = this;
		for (auto word : vec)
...
void LibraryCategory::AddLibraryEntry(const LibraryEntry& vLibraryEntry)
{
	...
	auto vec = ez::splitStringToVector(vLibraryEntry.second.categoryPath, '/');
	if (!vec.empty())
	{
		auto cat = this;
		for (auto word : vec)
